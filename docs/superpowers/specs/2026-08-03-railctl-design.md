...
| D8 | RailCom sanity | only when D4 failed with `NoAck` **and** D5 passed: read CV29 and CV28 over service mode | note naming CV29 bit 3 and CV28 = 3 |
| D9 | decoder identity | reads CV7, CV8, CV1, CV17, CV18, CV29, CV144 over the best path from D4/D5 | report lines |
...
| 144 | `prog_lock` | Programming lock. Non-zero blocks writes; never itself locked | |
...
If `pom_read` is false, `--mode pom` aborts with `PomReadUnsupportedError` (exit 16) and names the two remedies: enable RailCom on the decoder (CV29 bit 3 = 1, CV28 = 3), or use `--mode service` with the loco on the programming track. There is no silent fallback: falling back to service mode would read a *different* locomotive if two are on the layout. `--mode auto` resolves to `pom` when `pom_read` is true or unprobed, and to `service` when it is probed false.
...
4. Live CV144 (`PROGRAMMING_LOCK_CV`) must read 0. Non-zero aborts (exit 9); the user clears it, the tool does not.
...
- **D — CV144, last of all.** A non-zero lock written earlier would block every subsequent write including verification retries. CV144 is verified by read-back only, and the report says that no retry was possible.
...
1. **ZIMO sound-CV descriptions.** CV265, CV266, CV273–277, CV287, CV288, CV313, CV314 and CV395–397 are sound-project dependent, and the experimental regulation CVs 147/148/149 have shifted meaning between firmware generations. Can you supply the CV sheet that shipped with your decoder and its sound project before 0.1.0? Without it these descriptions ship as best-effort. The numbers are stable regardless, so this affects labels only.
